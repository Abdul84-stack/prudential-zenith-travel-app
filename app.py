...
def login():
    ...
    st.markdown('<h2 class="sub-header" style="text-align: center;">Travel Support Application</h2>', unsafe_allow_html=True)
...
def dashboard():
    ...
    # Sidebar navigation
    with st.sidebar:
        st.image("https://via.placeholder.com/150x50.png?text=Prudential+Zenith", width=150)
    ...
    if not recent_travel.empty:
        for _, row in recent_travel.iterrows():
            ...
            <div class="card {status_class}">
                <h4>{row['destination']} ({row['travel_type']})</h4>
                <p><strong>Purpose:</strong> {row['purpose']}</p>
                <p><strong>Dates:</strong> {row['departure_date']} to {row['arrival_date']}</p>
                <p><strong>Status:</strong> <span style="font-weight:bold">{row['status'].upper()}</span></p>
            </div>
            """, unsafe_allow_html=True)
    else:
